...
def mine():
    # we get the proof from the client
    req = request.get_json()

    if blockchain.valid_proof(json.dumps(blockchain.last_block, sort_keys=True), req["proof"]):
      # We must receive a reward for finding the proof.
      blockchain.new_transaction(sender='0', recipient=node_identifier, amount=1)

      # Forge the new Block by adding it to the chain
      previous_hash = blockchain.hash(blockchain.last_block)
      block = blockchain.new_block(req["proof"], previous_hash)
      ...
      return jsonify({"message": "Wrong Proof"}), 400
